<template>
  <a-form :label-col="{ style: { width: '80px' } }" label-align="left">
    ...
    <a-form-item label="MCP模式">
      <a-select v-model:value="formData.mcpType" :options="mcpTypes" placeholder="MCP模式" />
    </a-form-item>   
     <a-form-item label="MCP指令">
      <a-mentions v-model:value="formData.command" autofocus :options="parameters" placeholder="输入$补全" prefix="$"
           :rows="2"></a-mentions>
    </a-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      mcpTypes: [{ label: "本地模式", value: 1 }, { label: "网络模式", value: 2 }],
      parameters: [],
      formData: {
        parameterName: "tool",
        id: "",
  ...
  methods: {
    setForm() {
      ...
      this.formData.command = properties.command;
      this.formData.mcpType = properties.mcpType;
      this.formData.mcpNotice = properties.mcpNotice;
