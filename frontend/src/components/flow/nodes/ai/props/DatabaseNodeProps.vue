<template>
  <a-form :label-col="{ style: { width: '80px' } }" label-align="left">
    ...
    <a-form-item label="数据库">
      <a-select v-model:value="formData.sourceId" :options="sources" placeholder="数据库" />
    </a-form-item>

    <!-- 执行SQL -->
    <a-form-item label="执行SQL">
      <a-mentions v-model:value="formData.sql" autofocus :options="parameters" placeholder="输入$补全" prefix="$"
        :rows="5"></a-mentions>
    </a-form-item>
    ...
    <!-- 参数输入 -->
    <a-form-item label="参数输入">
      <a-mentions v-model:value="formData.args" autofocus :options="parameters" placeholder="输入$补全" prefix="$"
        :rows="3"></a-mentions>
    </a-form-item>
